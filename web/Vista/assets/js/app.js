...
var App = function () {
        ...
        switch ($mode) {
            ...
            case 'sidebar_close':
                if ($windowW > 991) {
                    ...
                    $('#backdrop').attr('style', 'display: none; z-index: 1031;');
            ...
            case 'sidebar_mini_toggle':
                if ($windowW > 991) {
                    ...
                    if (!$("#imgAvatar").hasClass('esconder')) {

                        $("#imgAvatar").addClass('esconder');

                    } else {

                        $("#imgAvatar").removeClass('esconder');
            ...
            case 'side_overlay_close':
                if ($windowW < 991) {
                    $('#backdropside').attr('style', 'display: none; z-index: 1042;');
                }
                var txtBandera = 10;
                //Pruebas
                $.ajax({
                    url: '../Transporte/queryCPreferencias.jsp',
                    data: '&txtBandera=' + txtBandera,
                    type: 'post',
    ...
    var uiHandleTheme = function () {
        ...
        jQuery('[data-toggle="theme"]').on('click', function () {
            ...
            //Se inserta tema en bd
            var tema = $theme;
            var txtBandera = 0;
            //alert(lstDomicilioDeudas);
            //Pruebas
            $.ajax({
                url: '../Transporte/queryCPreferencias.jsp',
                data: '&txtBandera=' + txtBandera + '&tema=' + tema,
                type: 'post',
                success: function (resp) {
                    //alert('I did it: '+resp);
                }
    ...
    var uiHelperMaxlength = function () {
        ...
        jQuery('.js-maxlength').each(function () {
            ...
            $input.maxlength({
                ...
                threshold: $input.data('threshold') ? $input.data('threshold') : 20,
                warningClass: $input.data('primary-class') ? $input.data('primary-class') : 'label label-primary',
...
//Cerrar modal de utilidades  
$('.CerraModalUtilidades').click(function () {
...
function mensajesInbox() {
    ...
    $.ajax({
        url: '../Transporte/queryCMensajesSinLeer.jsp',
        type: 'post',
        success: function (resp) {
            $("#numeroMensajesInbox").html(resp);
        }
    });
}
...
//    var heightfooter = $('#page-footer').height();
//    var heightTotal = heightbody - 40;
//    $('#main-container').attr("style", "height:" + heightTotal + "px !important; overflow:hidden;");


    $('#backdropside').click(function () {
        $lPages = jQuery('#page-container');
        var $windowW = window.innerWidth || document.documentElement.clientWidth || document.body.clientWidth;
        if ($windowW < 991) {
            $('#backdropside').attr('style', 'display: none; z-index: 1042;');
        }
        $lPages.removeClass('side-overlay-o');
    });
    /**
...
//alert('dysplay');
    ...
    //guardar anclar menu al momento de cambiar
    $("#checkAnclarMenu").change(function () {
        if ($(this).prop('checked') == true) {
            var checar = 1;
            $("#page-container").addClass("sidebar-mini");
            $("#imgAvatar").addClass('esconder');
            $("#imgAvatar").addClass('hideElement');
        } else {
            var checar = 0;
            $("#page-container").removeClass("sidebar-mini");
            $("#imgAvatar").removeClass('esconder');
            $("#imgAvatar").removeClass('hideElement');
        }
        $.ajax({
            url: '../Transporte/queryCPreferencias.jsp',
            data: '&txtBandera=4' + '&attMenu=' + checar,
            type: 'post',
            success: function (resp) {
                //alert('I did it: '+resp);
            }
    ...
    $('.ClassMinusZoom').click(function () {
//(window.parent.document.body.style.zoom.toString());
        var valorZoom = parseInt(window.parent.document.body.style.zoom);
        //alert(valorZoom);
        if (valorZoom > 60) {
            document.getElementsByTagName("body")[0].setAttribute("style", "zoom:" + (valorZoom - 10) + '%; overflow: hidden;');
            $('#LabelZoom').html((valorZoom - 10) + ' %');
            $('#LabelZoom2').html((valorZoom - 10) + ' %');
            var heightbody = $('body').height();
            var heightTotal = heightbody - 40;
            $('#main-container').attr("style", "height:" + heightTotal + "px !important;");
            //MarginBottom();
            var txtBandera = 1;
            var zoom = (valorZoom - 10);
            //Pruebas
            $.ajax({
                url: '../Transporte/queryCPreferencias.jsp',
                data: '&txtBandera=' + txtBandera + '&zoom=' + zoom,
                type: 'post',
                success: function (resp) {
                    //alert('I did it: '+resp);
                }
...
//alert((valorZoom + 10));
    ...
    $('.ClassZoom').click(function () {
        var valorZoom = parseInt(window.parent.document.body.style.zoom);
        document.getElementsByTagName("body")[0].setAttribute("style", "zoom:" + (valorZoom) + '%;');
        $('#LabelZoom').html((valorZoom) + ' %');
        $('#LabelZoom2').html((valorZoom) + ' %');
        var heightbody = $('body').height();
        var heightTotal = heightbody - 50;
        $('#main-container').attr("style", "height:" + heightTotal + "px !important;");
    });
    FooterZ();
    function FooterZ() {
    ...
    function MarginBottom() {
        ...
        if (valorZoom < 100) {
            ...
            $('body').find('.form-control').each(function () {
                var curRel = $(this).attr("style");
                if ($(this).is(":text")) {

                    var style = curRel + ' margin-bottom: -1px !important;';
                    //alert(style.replace('undefined',''));
                    $(this).attr('style', style.replace('undefined', ''));
                }
...
//Fincion para cambiar el avatar
    $("#TodosAvatares").on("click", ".avatar", function () {

        var ruta = $(this).attr("value");
        var fotoActual = $('.fotoSesion').attr("value");
        $.ajax({
            url: '../Transporte/queryCPreferencias.jsp',
            data: '&txtBandera=3&ruta=' + ruta,
            type: 'post',
            success: function (html) {

                $('body').find('.fotoSesion').removeAttr('src');
                $('body').find('.fotoSesion').attr('src', ruta);
            }
        });
    });
//    $("#modal-CambiarAvatar").on("click", ".configF", function () {
//
//        $("#LF").show();
//
//        $.ajax({
...
//    $("#modal-CambiarAvatar").on("click", ".configM", function () {
//
//        $("#LM").show();
//
//        $.ajax({
...
//    });
    ...
    $("#divCambiarAvatar").on("click", ".btnCambiarAvatar", function () {
    ...
    $("#sidebar").hover(function () {
        ...
        if ($("#imgAvatar").hasClass('esconder')) {
            ...
            $("#imgAvatar").toggleClass('hideElement');
        }
    }
    );
    ...
    //mensajesInbox();
    //*****************************************  Configuracion e inicializacion de plugin easy-autocomplete  *******************************************************

    var rol = $("#rolUsuario").val();
    if (rol === "Docente") {
        var options = {

            url: "../assets/js/plugins/easy-autocomplete/Docente.json",
            categories: [{
                    listLocation: "Docente",
                    maxNumberOfElements: 6,
                    header: "Busqueda Docente"
                }],
            ...
            template: {
                ...
                method: function (value, item) {
                    return "<a class='h3 text-primary-light' href='" + item.icon + "'><br>" + value + "</a>" + "<p class='h4'>" + item.desc + "</p>";
                }
            },
            requestDelay: 190,
            list: {
        ...
        $("#txtSearch").after('<span class="input-group-addon"><i class="si si-magnifier"></i></span>');
    } else if (rol === "Familiar") {
        var options = {

            url: "../assets/js/plugins/easy-autocomplete/Familiar.json",
            categories: [{
                    listLocation: "Familiar",
                    maxNumberOfElements: 6,
                    header: "Busqueda Familiar"
                }],
            getValue: function (element) {
                return element.titulo;
            },
            template: {
                type: "custom",
                method: function (value, item) {
                    return "<a class='h3 text-primary-light' href='" + item.icon + "'><br>" + value + "</a>" + item.desc;
                }
            },
            requestDelay: 190,
            list: {
        ...
        var options = {
            ...
            url: "../assets/js/plugins/easy-autocomplete/Alumno.json",
            categories: [{
                    listLocation: "Alumno",
                    maxNumberOfElements: 6,
                    header: "Busqueda Alumno"
                }],
            getValue: function (element) {
                return element.titulo;
            },
        ...
        var options = {
            ...
            url: "../assets/js/plugins/easy-autocomplete/Administrador.json",
            categories: [{
                    listLocation: "Administrador",
                    maxNumberOfElements: 6,
                    header: "Busqueda Administrador"
                }],
            getValue: function (element) {
                return element.titulo;
            },
            template: {
                type: "custom",
                method: function (value, item) {
                    return "<a class='h3 text-primary-light' href='" + item.icon + "'><br>" + value + "</a>" + "<p class='h5'>" + item.desc + "</p>";
                }
            },
            requestDelay: 190,
            list: {
    ...
    $("form[name='lblSearch']").submit(function (e) {
        e.preventDefault();
    ...
    var jspActual = $("#jspActual").val();
    if (jspActual !== '/Page404.jsp') {
        $(".easy-autocomplete").addClass("form-material form-material-primary input-group remove-margin-t remove-margin-b").removeAttr('style');
        $(".form-material-primary").css('width', '100%');
    }

//*************************************************************  fin de easy-Autocomplete  **************************************************




    $("body").on('click', "button[data-toggle='modal']", function () {
        var lenght_content_activo = $('.Window_Chat_Content').closest(".Chat_Show").length;
        if (lenght_content_activo === 1) {
            $('.Window_Chat_Content').hide();
        }
    });

    $("body").on('click', "button[data-toggle='tooltip']", function () {
        var lenght_content_activo = $('.Window_Chat_Content').closest(".Chat_Show").length;
        if (lenght_content_activo === 1) {
            $('.Window_Chat_Content').hide();
        }
    ...
    $("#btnAceptarConfig").click(function () {
...
//linea para quitar el autocompletado de los input
